...
class KynDynProxy:
    ...
    def __convert_attribute(self, value, name):
        ...
        if name in ["motion_subspaces", "joint_transforms", "forward_kinematics"]:
            return value
            ...
            case "jacobian_full_doubly_left":
                if (
                    ...
                    value = convert_jacobian(
                        ...
                        base_transform=self._kyn_dyn.velocity_representation,
    ...
    def __setattr__(self, name, value):
        ...
        self._kyn_dyn.replace(**{name: value})
...
class JaxSimModelData(common.ModelDataWithVelocityRepresentation):
    ...
    def build(
        ...
        base_transform = jnp.vstack(
            [
                jnp.block([base_orientation, base_position.reshape(3, -1)]),
                jnp.array([0, 0, 0, 1]),
            ]
        )
        ...
        J, W_H_L = jaxsim.rbda.jacobian_full_doubly_left(
            model=model,
            joint_positions=joint_positions,
            joint_transforms=i_X_λ,
            motion_subspaces=S,
        )
        J̇, _ = jaxsim.rbda.jacobian_derivative_full_doubly_left(
            model=model,
            joint_positions=joint_positions,
            joint_velocities=joint_velocities,
        )
